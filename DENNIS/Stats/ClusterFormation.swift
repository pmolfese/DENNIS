...
//  onto the lattice.
//
//  Note: "cluster" here means a connected set of channel x time points in a
//  statistical map. That is unrelated to the PCA spatial-loading channel
//  clusters in `Viz/ClusterERPView.swift`, which share the word and nothing
//  else.
//
//  References (full citations in `References.swift`):
//    - Maris & Oostenveld (2007), J Neurosci Methods 164(1):177-190 — cluster
//      mass, and the maximum-cluster-statistic null used to correct it.
//    - Smith & Nichols (2009), NeuroImage 44(1):83-98 — TFCE, and the default
//      E = 0.5 / H = 2 exponents.
//    - Nichols & Holmes (2002), Hum Brain Mapp 15(1):1-25 — the max-statistic
//      permutation framework these two share.
//    - Phipson & Smyth (2010), SAGMB 9(1):39 — the (b+1)/(m+1) Monte-Carlo
//      p-value in `ClusterCorrection`.
//    - Ernst (2004), Statist Sci 19(4):676-685 — the exact p-value used instead
//      when the null is enumerated rather than sampled.
//
...
nonisolated enum ClusterFormingThreshold: Sendable, Equatable, Hashable {
...
nonisolated enum ClusterCorrection {
    /// Permutation p-value for one observed statistic.
    ///
    /// Under Monte-Carlo sampling the observed arrangement is added to its own
    /// null, so the smallest attainable p is `1 / (permutations + 1)` and the
    /// test stays exact rather than reporting an impossible p = 0. Under
    /// *exhaustive* enumeration the observed arrangement is already a member of
    /// the enumerated null, so adding it again would double-count it: the
    /// correct form is `#{null >= observed} / count`, with a floor of `1/count`.
    static func pValue(observed: Double, nullMaxima: [Double], exhaustive: Bool) -> Double {
        guard !nullMaxima.isEmpty else { return 1 }
        let exceedances = nullMaxima.reduce(into: 0) { count, value in
            if value >= observed { count += 1 }
        }
        if exhaustive {
            // The identity arrangement is enumerated, and its maximum is >= any
            // observed cluster's mass, so `exceedances` is never zero in
            // practice; the clamp only guards against a degenerate null.
            return Double(max(exceedances, 1)) / Double(nullMaxima.count)
        }
        return Double(exceedances + 1) / Double(nullMaxima.count + 1)
    }

    /// Point-wise corrected p from the max-statistic null, used by TFCE.
    static func pointPValues(scores: [Double], nullMaxima: [Double], exhaustive: Bool) -> [Double] {
        guard !nullMaxima.isEmpty else { return [Double](repeating: 1, count: scores.count) }
        let sorted = nullMaxima.sorted()
        let denominator = exhaustive ? Double(sorted.count) : Double(sorted.count + 1)
        return scores.map { score in
            ...
            let exceedances = sorted.count - low
            let numerator = exhaustive ? Double(max(exceedances, 1)) : Double(exceedances + 1)
