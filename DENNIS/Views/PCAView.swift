...
//  DENNIS
//
//  PCA mode for a selected group: grand-average preview, the PCA trim window,
//  temporal scree + temporal PCA, and the dual (temporal → spatial) decomposition.
//  Rendering and user inputs live here; the analysis runs are delegated to
//  PCAAnalysisModel so they stay testable and off the view.
//
...
struct PCAView: View {
    ...
    @State private var model = PCAAnalysisModel()
    ...
    private var temporalScreeColumn: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            } else if let analysis = model.screeAnalysis {
    ...
    private var temporalLoadingsColumn: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if conditionNames.isEmpty {
                Text("No shared conditions yet.").font(.caption).foregroundStyle(.secondary)
            } else if let error = model.pcaError {
                Text(error).font(.caption).foregroundStyle(.red)
    ...
    private var dualPCASection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 6) {
                ...
                Button {
                    let axis = currentTimeAxis()
                    model.runDualPCA(members: members, conditionNames: conditionNames,
                                     timeIndices: axis?.indices, timesMS: axis?.timesMS ?? [],
            ...
            if model.dualRunning { progressBar(model.dualProgress) }

            // Combined-factor variance and spatial scree, side by side.
            if model.dualModel != nil || model.spatialScree != nil || model.spatialScreeError != nil {
                HStack(alignment: .top, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            Spacer()
                            if let scree = model.spatialScree {
                                savePNGButton("spatial_scree") { ScreePlotView(analysis: scree) }
                            }
                        }
                        if let error = model.spatialScreeError {
                            Text(error).font(.caption).foregroundStyle(.red)
                        } else if let scree = model.spatialScree {
            ...
            if conditionNames.isEmpty {
                Text("No shared conditions across these subjects yet (still loading?).")
                    .font(.caption).foregroundStyle(.secondary)
            } else if let error = model.dualError {
                Text(error).font(.caption).foregroundStyle(.red)
                ...
                DualPCAView(result: dualModel, sensorLayout: groupSensorLayout,
                            ...
                            clusterBaseline: model.clusterBaseline,
                            clusterSamplingRate: model.clusterSamplingRate)
            }
        }
    }
