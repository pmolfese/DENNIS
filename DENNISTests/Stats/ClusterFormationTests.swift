...
//  the same integral. The naive version is what caught a real charge-propagation
//  bug in the fast one, so it is kept rather than trimmed.
//
...
struct ClusterFormationTests {
    ...
    @Test func tfceMatchesTheNaiveIntegralOnSignedData() {
        var rng = SplitMix64(seed: 0x7FCE_0001)
    ...
    @Test func tfceMatchesTheNaiveIntegralOnNonNegativeData() {
        var rng = SplitMix64(seed: 0x7FCE_0002)
    ...
    @Test func monteCarloPValueIncludesTheObservedStatisticInItsOwnNull() {
        // With 99 sampled permutations the floor is 1/100, never zero.
        let null = [Double](repeating: 0, count: 99)
        #expect(abs(ClusterCorrection.pValue(observed: 100, nullMaxima: null, exhaustive: false) - 0.01) < 1e-12)
        let dominating = [Double](repeating: 500, count: 99)
        #expect(abs(ClusterCorrection.pValue(observed: 1, nullMaxima: dominating, exhaustive: false) - 1.0) < 1e-12)
    }

    @Test func exhaustivePValueDividesByTheEnumeratedCount() {
        // Under enumeration the observed arrangement is already in the null, so
        // adding it again would double-count it: the floor is 1/count, not
        // 1/(count + 1).
        var null = [Double](repeating: 0, count: 256)
        null[0] = 100      // the identity arrangement
        #expect(abs(ClusterCorrection.pValue(observed: 100, nullMaxima: null, exhaustive: true) - 1.0 / 256) < 1e-12)
        #expect(ClusterCorrection.pValue(observed: 100, nullMaxima: null, exhaustive: true)
                < ClusterCorrection.pValue(observed: 100, nullMaxima: null, exhaustive: false))
    }

    @Test func pointPValuesMatchADirectCount() {
        var rng = SplitMix64(seed: 0x9911)
        let null = (0..<200).map { _ in Double.random(in: 0...10, using: &rng) }
        let scores = (0..<50).map { _ in Double.random(in: -10...10, using: &rng) }
        let computed = ClusterCorrection.pointPValues(scores: scores, nullMaxima: null, exhaustive: false)
        ...
        let exhaustive = ClusterCorrection.pointPValues(scores: scores, nullMaxima: null, exhaustive: true)
        for (index, score) in scores.enumerated() {
            let exceedances = max(null.filter { $0 >= abs(score) }.count, 1)
            #expect(abs(exhaustive[index] - Double(exceedances) / Double(null.count)) < 1e-12)
        }
    }
